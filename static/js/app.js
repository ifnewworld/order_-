...
function isCompleted(item) {
  return item.shipping_to_korea && item.factory_arrived && item.photo_taken && item.coupang_wing_registered && item.rocket_inbound_requested && item.rocket_inbound_completed;
}
...
function setStatusFilter(filter) {
  statusFilter = filter;
  ['filterAll', 'filterKorea', 'filterFactory', 'filterRocket', 'filterPhoto', 'filterInboundReq', 'filterInboundDone'].forEach(id => {
    document.getElementById(id)?.classList.remove('stat-card-active');
  });
  const idMap = { all: 'filterAll', korea: 'filterKorea', factory: 'filterFactory', photo: 'filterPhoto', rocket: 'filterRocket', inbound_req: 'filterInboundReq', inbound_done: 'filterInboundDone' };
...
function renderTable() {
  ...
  let list = items.filter(i => {
    ...
    else if (statusFilter === 'inbound_req')  matchStatus = i.rocket_inbound_requested === false;
    else if (statusFilter === 'inbound_done') matchStatus = i.rocket_inbound_completed === false;
  ...
  if (list.length === 0) {
    ...
    const dbOrderMap = {};
    sortedByCreated.forEach((item, idx) => { dbOrderMap[item.id] = idx + 1; });
    tbody.innerHTML = list.map(item => rowHTML(item, dups, dbOrderMap[item.id])).join('');
  }
...
function rowHTML(item, dups, rowIndex) {
  ...
  return `
    <tr class="${rowClass}">
      ...
      <td style="cursor:pointer" onclick="toggleField('${item.id}','rocket_inbound_requested',${!!item.rocket_inbound_requested})">${yesNo(item.rocket_inbound_requested)}</td>
      <td style="cursor:pointer" onclick="toggleField('${item.id}','rocket_inbound_completed',${!!item.rocket_inbound_completed})">${yesNo(item.rocket_inbound_completed)}</td>
...
function updateStats(dups) {
  ...
  document.getElementById('statInboundReq').textContent  = items.filter(i => i.rocket_inbound_requested === false).length;
  document.getElementById('statInboundDone').textContent = items.filter(i => i.rocket_inbound_completed === false).length;
}
...
function openModal(id = null) {
  ...
  document.getElementById('fPhoto').value     = String(item?.photo_taken ?? false);
  document.getElementById('fCouReg').value    = String(item?.coupang_wing_registered ?? false);
  document.getElementById('fInboundReq').value  = String(item?.rocket_inbound_requested ?? false);
  document.getElementById('fInboundDone').value = String(item?.rocket_inbound_completed ?? false);
...
async function saveItem() {
  ...
  const payload = {
    ...
    photo_taken:            document.getElementById('fPhoto').value === 'true',
    coupang_wing_registered: document.getElementById('fCouReg').value === 'true',
    rocket_inbound_requested:  document.getElementById('fInboundReq').value === 'true',
    rocket_inbound_completed:  document.getElementById('fInboundDone').value === 'true',
...
function exportXLSX() {
  const headers = ['SKU 코드','상품명','카테고리','주문 수량','파손 물량','사용 가능 물량',
                   '한국으로 오는 중','주문 날짜','공장 도착','공장 검수','로켓그로스 등록','쿠팡윙 등록','로켓그로스 입고 신청','로켓그로스 입고 완료','비고','등록일'];
  const rows = sortItems([...items]).map(i => [
    ...
    i.rocket_inbound_requested    ? 'Y' : 'N',
    i.rocket_inbound_completed    ? 'Y' : 'N',
    i.note ?? '',
...
const FIELD_LABELS = {
  ...
  rocket_inbound_requested: '로켓그로스 입고 신청',
  rocket_inbound_completed: '로켓그로스 입고 완료',
  note: '비고',
};
...
async function loadPhotos(orderId) {
  ...
  if (!photos.length) {
    grid.innerHTML = '<span style="font-size:12px;color:var(--text-muted,#9ca3af)">아직 사진이 없습니다.</span>';
    return;
  }
  grid.innerHTML = photos.map(p => `
    <div style="position:relative;width:80px;height:80px">
    <img src="${p.url}" style="width:80px;height:80px;object-fit:cover;border-radius:6px;border:1px solid var(--border,#e5e7eb)" />
    <button onclick="deletePhoto('${p.id}','${p.order_id}')"
      style="position:absolute;top:2px;right:2px;width:18px;height:18px;border-radius:50%;border:none;
             background:rgba(0,0,0,.55);color:#fff;font-size:11px;cursor:pointer;line-height:1">×</button>
    <a href="/api/photos/${p.id}/download"
      style="position:absolute;bottom:2px;right:2px;width:18px;height:18px;border-radius:50%;
            background:rgba(0,0,0,.55);color:#fff;font-size:11px;cursor:pointer;line-height:18px;
            text-align:center;text-decoration:none;display:block" title="다운로드">↓</a>
  </div>`).join('');
}

async function handlePhotoSelect(input) {
  if (!input.files.length) return;
  const label = document.getElementById('photoFileLabel');
  label.textContent = '업로드 중...';

  for (const file of Array.from(input.files)) {
    const fd = new FormData();
    fd.append('file', file);
    fd.append('uploaded_by', currentUser || '');
    const res = await fetch(`/api/photos/${editId}`, { method: 'POST', body: fd });
    if (!res.ok) {
      const err = await res.json();
      showToast(`업로드 실패: ${err.detail}`, 'error');
    }
  }

  input.value = '';
  label.textContent = '선택된 파일 없음';
  await loadPhotos(editId);
}
